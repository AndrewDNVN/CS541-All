...
class nameNode extends exprNode {
	...
	void cg(){
		...
		if(varName.idinfo.varIndex != -42){
			...
			String type = varName.type.toString(); 

			//
			gen("putstatic", packageNameID + "/" + varName.idname, type);
		}
	}
...
//^ the above have issues that I could not diagnose before I 
// needed to move on for other class work
...
class fctCallNode extends exprNode {
	...
	void cg(){
		...
		// call to the methode with the given packagename / the signature of the method
		gen("invokestatic", packageNameID +"/"+funcName.varName.idinfo.signature);
	}
...
class callNode extends stmtNode {
	...
	void cg(){
		...
		// same as the function call node
...
class castNode extends exprNode {
	...
	void cg(){
		...
		// check if the result needs to be bool
		if(resultType.type.val == Types.Boolean){
			
			if (operand.type.val == Types.Integer || operand.type.val == Types.Character){
				gen("; generating code for casting from int or char");
				opGen("ifne");
				// that would load either a 1 or a zero on the stack
			}
		...
		if(resultType.type.val == Types.Character){
			...
			if(operand.type.val == Types.Integer){
				...
				gen("; generating code for casting from int to char");
				gen("i2c");
